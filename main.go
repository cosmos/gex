...
var givenHost = flag.String("h", "localhost", "host to connect to as a string")
var useSsl = flag.Bool("s", false, "protocol (http/https, ws/wss) to use as a boolean")
...
func main() {
	...
	if !networkStatus.Exists() {
		panic("Application not running on " + fmt.Sprintf("%s://%s:%s", httpProtocol, *givenHost, *givenPort))
	}
...
func writeBlocks(ctx context.Context, info Info, t *text.Text, connectionSignal <-chan string) {
	...
	socket := gowebsocket.New(fmt.Sprintf("%s://%s:%s/websocket", protocol, host, port))
...
func getFromRPC(endpoint string) string {
	...
	resp, _ := resty.R().
		...
		Get(fmt.Sprintf("%s://%s:%s/%s", protocol, host, port, endpoint))
